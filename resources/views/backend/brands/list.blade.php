...
@section('content')
                                        ...
                                        @foreach ($brands as $item)
                                            <tr data-order="{{ $item->id }}">
                                                ...
                                                <td>
                                                    @if ($item->is_active)
                                                        <span
                                                            class="chip lighten-5 green green-text">{{ __('Published') }}</span>
                                                    @else
                                                        <span
                                                ...
                                                <td>
                                                    <span
                                                        class="chip lighten-5 green green-text">{{ App\Enums\BrandType::fromValue($item->status)->description }}</span>
                                                </td>
                                                <td>
                                                ...
                                                <td class="center-align">
                                                    <a href="{{ route('admin.brand.show', $item) }}"
                                                        class="btn-floating cyan pulse"><i
                                                            class="material-icons">edit</i></a>
                                                    <a href="#modalDelete"
                                                        onclick="setRoute('{{ $item }}', '{{ route('admin.brand.destroy', $item) }}')"
    ...
    <div id="modalDelete" class="modal">
        <div class="modal-content">
            <h4>{{ __('admin-content.delete-brand') }}</h4>
            <p>{{ __('admin-content.are-you-sure-you-need-to-delete-this') }} {{ __('admin-content.brand') }}?</p>
        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#save_order').click(function(e) {
                ...
                var newForm = $('<form>', {
                        'action': "{{ route('admin.brand.order.save') }}",
                        'method': 'POST'
                    })
                    .append($('<input>', {
                        'name': 'orders',
