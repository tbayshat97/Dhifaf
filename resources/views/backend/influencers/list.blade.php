...
@section('content')
                                ...
                                <table id="page-length-option" class="display">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ __('admin-content.username') }}</th>
                                            <th>{{ __('admin-content.name') }}</th>
                                            <th>{{ __('admin-content.phone') }}</th>
                                    ...
                                    <tbody>
                                        @foreach ($influencers as $item)
                                            <tr>
                                            <td>{{ $item->id }}</td>
                                            <td><img src="{{ $item->profile->getProfileImage() }}" width="75" height="75"
                                                    class="" /> </td>
                                            <td>{{ $item->username}}</td>
                                            ...
                                            <td class="center-align">
                                                <a href="{{ route('admin.influencer.show', $item) }}" class="btn-floating cyan pulse"><i class="material-icons">edit</i></a>
                                                <a href="#modalDelete" onclick="setRoute('{{ $item }}', '{{ route('admin.influencer.destroy', $item) }}')" class="modal-trigger btn-floating red pulse"><i class="material-icons">delete</i></a>
                                            </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
    ...
    <div id="modalDelete" class="modal">
        <div class="modal-content">
            <h4>{{ __('admin-content.delete-influencer') }}</h4>
            <p>{{ __('admin-content.are-you-sure-you-need-to-delete-this') }} {{ __('admin-content.influencer') }}?</p>
        </div>
