                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="center-align">
                                            <a href="{{ route('influencer.course.show', $item) }}"
                                                class="btn-floating cyan pulse"><i class="material-icons">edit</i></a>
                                                @if (!$item->live)
                                                <a href='javascript:live("{{ route('influencer.course.live.store', $item)
                                                    }}");' class="btn-floating tooltipped  cyan green pulse"
                                                    data-position="top" data-tooltip="Go Live"><i
                                                        class="material-icons">videocam</i></a>
                                                @endif
                                            <a href="#modalDelete"
                                                onclick="setRoute('{{ $item->id }}', '{{ route('influencer.course.destroy', $item) }}')"
...
<script>
    ...
    function live(route) {
        swal({
            title: "Are you sure you want make this live?",
            icon: 'warning',
            dangerMode: true,
            buttons: {
                cancel: 'No, Please!',
            ...
            if (willDelete) {
                $.ajax({
                    ...
                    success: function (response) {
                        setTimeout(goTo("{{ route('influencer.course.live.list') }}"), 2000);
                    }
                });
            } else {
